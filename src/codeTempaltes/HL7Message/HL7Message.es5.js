...
HL7Message.parser = function (hl7, encoding) {
    ...
    if (typeof hl7 !== 'string') {
        throw new Error('Invalid Message! Message must be XML or text! Found: ' + typeof hl7)
    }
...
HL7Message.prototype.set = function (path, value, autoResolve) {
    try {
        ...
        e.message = 'Failed to set ' + path + ' to ' + errVal + '\n' + e.message
        throw e
    }
}
/**
...
HL7Message.prototype.toXML = function () {
    ...
    const raw = this.toString().replace(/null/g, '').replace(/undefined/g, '').replace(/[^\r\n\x20-\x7E]+/g, ' ')
