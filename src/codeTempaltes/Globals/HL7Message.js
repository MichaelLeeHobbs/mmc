/**
 * HL7Message - a self-contained HL7 v2 message parser/builder for Mirth Connect / Rhino.
 *
 * Single-file bundle of two classes: Encoding (the encoding-character helper) and
 * HL7Message (which depends on it), Encoding first. In Mirth, paste this one code
 * template and use `new HL7Message(...)`; the `Encoding` class is also exposed.
 *
 * Requires Rhino languageVersion >= 180 (uses `let` and object destructuring), i.e.
 * Mirth's `rhinoLanguageVersion` server setting must be >= 180. See
 * docs/RHINO-COMPATIBILITY.md.
 *
 * Full API with many examples: docs/HL7Message.md.
 *
 * @example
 * var msg = new HL7Message($('rawMessage'))
...
// Node.js interop (also used by the repo's vm test harness). Exposes HL7Message as
// the default export with Encoding attached; both are global in Mirth/Rhino.
if (typeof module !== 'undefined') {
    module.exports = HL7Message
    module.exports.Encoding = Encoding
}

/* exported HL7Message, Encoding */
